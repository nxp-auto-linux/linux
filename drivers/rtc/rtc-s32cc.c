 ...
 */
static int s32cc_priv_setup(struct rtc_s32cc_priv *priv)
{
	u32 rtcc = 0;
	u32 clksel;
	...
	case RTCC_CLKSEL(S32CC_RTC_SOURCE_SIRC):
		priv->rtc_hz = S32CC_SIRC_HZ;
		break;
	case RTCC_CLKSEL(S32CC_RTC_SOURCE_FIRC):
		priv->rtc_hz = S32CC_FIRC_HZ;
		break;
	default:
		return -EINVAL;
	}
...
static int s32cc_rtc_probe(struct platform_device *pdev)
{
	 ...
	 */
	priv->div512 = true;
	priv->clk_source = S32CC_RTC_SOURCE_FIRC;
	if (s32cc_priv_setup(priv)) {
		err = -EINVAL;
		goto err_priv_setup;
	}
