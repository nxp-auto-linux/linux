...
/* handling phy interrupts */
static irqreturn_t nxp_handle_interrupt(struct phy_device *phydev)
{
				   ...
				   const char *buf, size_t count)
{
	...
	case 0:
		...
		break;
	case 1:
		/* disable link control before entering test */
		set_link_control(phydev, 0);
...
static struct phy_driver nxp_drivers[] = {
	{
	 ...
	 .handle_interrupt = &nxp_handle_interrupt,
	 },
	{
				 ...
				 const void *desired_name)
{
	 ...
	 * it's state should be MDIO_REGISTERED at this point. If it is not,
	 * it is either not a 'struct mii_bus', either it is in an
	 * undesired state.
	 */
